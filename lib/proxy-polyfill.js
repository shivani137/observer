...
function Proxy(target, handler, revocable) {
    ...
    var get, set;
    if ('get' in handler) get = function (property) {
        return handler.get(target, property, this);
    };else get = function (property) {
        return target[property];
    };

    if ('set' in handler) set = function (property, value) {
        handler.set(target, property, value, this);
    };else set = function (property, value) {
    ...
    selfObserver.dontIgnore = function () {
        ...
        for (var i in events) if (selfObserver.ignore[name].includes(events[i])) selfObserver.ignore[name].splice(selfObserver.ignore[name].indexOf(events[i]), 1);
    ...
    targetObserver.dontIgnore = function () {
        ...
        var events = Array.prototype.slice(arguments, 1);
        for (var i in events) if (targetObserver.ignore[name].includes(events[i])) targetObserver.ignore[name].splice(targetObserver.ignore[name].indexOf(events[i]), 1);
